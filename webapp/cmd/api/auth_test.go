...
func Test_app_getTokenFromHeaderAndVerify(t *testing.T) {
	...
	var tests = []struct {
		...
		{"valid-expired-token", fmt.Sprintf("Bearer %s", expiredToken), true, true, app.Domain},
		{"no header", "", true, false, app.Domain},
		{"invalid token", fmt.Sprintf("Bearer %sasdasdasd", tokens.Token), true, true, app.Domain},
		{"no bearer", fmt.Sprintf("Bear %sasdasdasd", tokens.Token), true, true, app.Domain},
		{"three header parts", fmt.Sprintf("Bearer %s asdasdasd", tokens.Token), true, true, app.Domain},
		//make sure the next test is the last one to run
		{"wrong issues", fmt.Sprintf("Bearer %s", tokens.Token), true, true, "wrongdomain.com"},
	}
